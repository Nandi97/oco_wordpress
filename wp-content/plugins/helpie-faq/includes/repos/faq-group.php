...
if (!class_exists('\HelpieFaq\Includes\Repos\Faq_Group')) {
    class Faq_Group
    {
        ...
        public function set_category_terms($post_id, $faq_category_ids)
        {
            ...
            $term_ids = array_map(function ($id) {return (int) $id;}, $faq_category_ids);
        ...
        public function get_categories_with_posts_data_by_term_meta($group_category_ids, $term_meta)
        {
            ...
            for ($ii = 0; $ii < count($group_category_terms); $ii++) {
                ...
                $posts = array();
                if (!empty($post_ids)) {
                    $posts = get_posts(array('post__in' => $post_ids, 'post_type' => HELPIE_FAQ_POST_TYPE, 'numberposts' => -1, 'orderby' => 'post__in'));
                }
        ...
        public function get_pending_post_ids($term_id)
        {
            ...
            $posts = get_posts(array('post__in' => $post_ids, 'post_type' => HELPIE_FAQ_POST_TYPE, 'numberposts' => -1, 'orderby' => 'post__in', 'post_status' => 'pending'));
            if (!empty($posts)) {
