...
if(in_array('comment_count', $items) && $comment){
    $metaHtmlbottom .= '<span class="comment-count"><a href="' . get_comments_link() .'"><i class="fas fa-comments"></i> '. $num_comments.'</a></span>';
}
$postMetaBottom = null;
...
$html .= sprintf('<div class="%s" data-id="%d">', esc_attr(implode(" ", array_filter([$grid, $class, $isoFilter]))), $pID);
	$html .= sprintf('<div class="rt-holder%s">',$tpg_title_position ? " rt-with-title" : null);
		...
		if($imgSrc) {
			...
			$html .= "<a data-id='{$pID}' class='{$anchorClass}' href='{$pLink}'>$imgSrc</a>";
			$html .= '</div> ';
		}
