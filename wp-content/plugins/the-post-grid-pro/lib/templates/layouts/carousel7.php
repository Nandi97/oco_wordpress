...
if(in_array('excerpt', $items)){
    $excerptContent = "<p class='tpg-excerpt'>{$excerpt}</p>";
}
$postMetaBottom = null;
...
if(!empty($postMetaBottom)){
    $bottomContent = "<div class='post-meta {$btn_alignment_class}'>$postMetaBottom</div>";
}
...
$html .= sprintf('<div class="%s" data-id="%d">', esc_attr(implode(" ", [$grid, $class])), $pID);
    $html .= '<div class="rt-holder">';
        $html .= "<div class='overlay'>";
                ...
                $html .= "<div class='post-info'>{$titleHtml} <span class='post-meta-user'>{$metaHtml}</span>{$excerptContent}{$bottomContent}</div>";
        $html .="</div>";
    $html .= '</div>';
$html .='</div>';
